...
TinyString& TinyString::operator=(const TinyString& value)
{
...
TinyString& TinyString::operator=(const wchar_t* lpString)
{
	if(lpString == null)
	{
		release();
...
bool TinyString::getCharAt(uint index, wchar_t* lpChar_out)
{
	if(index >= m_lpStringMemory->length)
	{
		return false;
	}
	else
	{
		*lpChar_out = m_lpStringMemory->lpStr[index];
		return true;
	}
}
